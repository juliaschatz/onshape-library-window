...
var getAssemblyListVersion = function(req, res) {
    var url = apiUrl + '/api/documents/d/' + req.query.documentId + '/v/' + req.query.versionId + '/elements?elementType=assembly';
    if (req.query.elementId) {
      url += '/?elementId=' + req.query.elementId;
    }
    ...
    request.get({
      ...
      if (data.statusCode === 401) {
        authentication.refreshOAuthToken(req, res).then(function() {
          getAssemblyListVersion(req, res);
        }).catch(function(err) {
          console.log('Error refreshing token or getting elements: ', err);
        });
      } else {
        console.log('GET /api/documents/elements_version ?type=assembly error: ', data);
      }
    });
  };

var getPartStudiosVersion = function(req, res) {
  var url = apiUrl + '/api/documents/d/' + req.query.documentId + '/v/' + req.query.versionId + '/elements?elementType=partstudio';
  if (req.query.elementId) {
    url += '/?elementId=' + req.query.elementId;
  }
  ...
  request.get({
    ...
    console.log('****** getPartStudios - CATCH ' + data.statusCode);
    if (data.statusCode === 401) {
      authentication.refreshOAuthToken(req, res).then(function() {
        getPartStudiosVersion(req, res);
      }).catch(function(err) {
        console.log('Error refreshing token or getting elements: ', err);
      });
    } else {
      console.log('GET /api/documents/elements_version ?type=partstudio error: ', data);
    }
  });
};

var createAssemblyInstance = function(req, res) {
  var url = apiUrl + '/api/assemblies/d/' + req.query.documentId + '/w/' + req.query.workspaceId + '/e/' + req.query.elementId + '/instances';
  ...
  request.post({
      ...
      if (data.statusCode === 401) {
      authentication.refreshOAuthToken(req, res).then(function() {
          createAssemblyInstance(req, res);
      }).catch(function(err) {
          console.log('Error refreshing token or getting elements: ', err);
      });
      } else {
      console.log('POST /api/assemblies/ error: ', data);
      }
  });
};

var getAssemblyViews = function(req, res) {
  var url = apiUrl + '/api/assemblies/d/' + req.query.documentId + '/v/' + req.query.versionId + '/e/' + req.query.elementId + '/shadedViews';
  ...
  request.get({
      ...
      console.log('****** getAssemblyViews - CATCH ' + data.statusCode);
      if (data.statusCode === 401) {
      authentication.refreshOAuthToken(req, res).then(function() {
          getAssemblyViews(req, res);
      }).catch(function(err) {
          console.log('Error refreshing token or getting elements: ', err);
      });
      } else {
      console.log('GET /api/assembly_views error: ', data);
      }
  });
};
...
var getPartsStudio = function(req, res) {
  request.get({
    uri: apiUrl + '/api/parts/d/' + req.query.documentId + '/v/' + req.query.versionId + '/e/' + req.query.elementId,
    headers: {
      'Authorization': 'Bearer ' + req.user.accessToken
    }
  }).then(function(data) {
...
var getPartsListVersion = function(req, res) {
  request.get({
    uri: apiUrl + '/api/parts/d/' + req.query.documentId + '/v/' + req.query.versionId,
    headers: {
      'Authorization': 'Bearer ' + req.user.accessToken
    }
  }).then(function(data) {
...
var getPartsListVersion = function(req, res) {
    request.get({
      ...
      if (data.statusCode === 401) {
        ...
        console.log('GET /api/parts/version error: ', data);
      }
    });
  };
...
router.get('/assemblies_version', getAssemblyListVersion);
router.get('/partstudios_version', getPartStudiosVersion);
router.get('/parts_studio', getPartsStudio);
router.get('/assembly_views', getAssemblyViews);
router.get('/parts_version', getPartsListVersion);
router.post('/insert_assembly', createAssemblyInstance)
